	...
	SubShader {
		Tags { "Queue" = "Transparent" "IgnoreProjector" = "True"  }
			...
			void surf(Input IN, inout SurfaceOutput o)
			{
				...
				o.Emission = clamp(gradient * pow(rim, (1 - noise + 0.5 * _Noise) * _Fresnel) * pow(rim2, (1 - noise + 0.5 * _Noise) * _FresnelInvert) * fade * noise * _Brightness * IN.color.rgb, 0, _ClipBrightness);
